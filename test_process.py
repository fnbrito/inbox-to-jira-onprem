...
def cfg():
    return types.SimpleNamespace(
        project_key="BISSB", issue_type="Support Request", label_prefix="eml",
        skip_auto_replies=True, mark_read=True, upload_attachments=True,
        jira_user="svc-bishelp", mailbox="bishelp@maplesoft.com",
...
class ProcessOnceTest(unittest.TestCase):
    def setUp(self):
        ...
        e2j.fetch_unread = lambda c, t, top=None: list(self.inbox)
        ...
        def fake_create(c, s, fields):
            key = f"BISSB-{len(self.created)+1}"
    ...
    def test_create_skip_and_thread(self):
        ...
        self.assertEqual(self.comments[0][0], "BISSB-1")
